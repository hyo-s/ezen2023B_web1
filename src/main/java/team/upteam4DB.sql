...
insert into class(classname, classtype) values('컴퓨터구조', '전공필수');
insert into class(classname, classtype) values('대학수학', '교양필수');
insert into class(classname, classtype) values('대학수학2', '교양필수');
insert into class(classname, classtype) values('대학물리', '교양필수');
insert into class(classname, classtype) values('대학물리2', '교양필수');
insert into class(classname, classtype) values('네트워크실험', '전공선택');
insert into class(classname, classtype) values('컴퓨터보안', '전공필수');
...
insert into member(id, pw, name, phone, birth, state) values('aa','aa','김교수','010-1111-1111','19700101', 2);
insert into member(id, pw, name, phone, birth, state) values('bb','bb','박교수','010-2222-2222','19800202', 2);
insert into member(id, pw, name, phone, birth, state) values('cc','cc','최학생','010-3333-3333','19980303', 1);
insert into member(id, pw, name, phone, birth, state) values('dd','dd','나행정','010-4444-4444','19920404', 3);
insert into member(id, pw, name, phone, birth, state) values('ee','ee','이행정','010-5555-5555','19820505', 3);
...
insert into member(id, pw, name, phone, birth, state) values('admin','admin','admin','010-0000-0000','00000000', 4);
select * from member;
...
insert into classroom(roomnumber, totalperson) values(301,50);
...
insert into professor(pgrade, degree, majorpart, mainmajor,mno_fk) values('정교수','ph.D','네트워크엔지니어링','컴퓨터공학',1);
#insert into professor(pgrade, psalary, proom, degree, majorpart, mainmajor, mno_fk) values('aa',1,'aa','aa','aa','aa',1);
insert into professor(pgrade, degree, majorpart, mainmajor,mno_fk) values('부교수','ph.D','컴퓨터보안','컴퓨터공학',2);
...
insert into classinfo(classno, professor, roomnumber, tno, sno) values(2, 2, 2, 2, 2);
...
select i.no, c.classname, i.professor, m.name, r.roomnumber, t.dayweek, t.starttime, t.endtime, s.semester, c.cstate from classinfo i
...
insert into petition (ppno, ptitle, pcontent,participation,regidate ,duedate) value (1,'흡연실을 설치해 주세요','사람들이 길거리 흡연해서 너무 힘들어요 교내 흡연실을 설치해 주세요',499,'2024-02-19' ,'2024-04-17');
insert into petition (ppno, ptitle, pcontent,participation, duedate) value (2,'방학기간을 늘려주세요','방학이 너무 짧다',2, '2024-04-17');
insert into petition (ppno, ptitle, pcontent,participation, duedate) value (3,'수업이 너무 적습니다.','개설된 수업이 너무 적습니다.',410, '2024-04-17');
insert into petition (ppno, ptitle, pcontent,participation, duedate) value (4,'학생회비 내역 공개','학생회비 내역을 공개해 주세요',15, '2024-04-17');
insert into petition (ppno, ptitle, pcontent,participation, duedate) value (5,'학교 주변에 뭐가 너무 없습니다.','여가 생활을 즐기기 위한 무언가를 만들어주세요',99, '2024-04-17');
...
# =========================== 학사 일정 =========================== #
drop table if exists unischedule;
create table unischedule(
   scno int auto_increment,  # 식별번호
   stdate char(10),        # 시작날짜
    sccontent varchar(100),   # 내용
    scolor varchar(20),        # 배경색
    constraint unischedule_scno_pk primary key(scno)
);

select * from unischedule;
insert into unischedule (stdate, sccontent, scolor) value ('2024-05-05',  '어린이날', 'black') ;
insert into unischedule (stdate, sccontent, scolor) value ('2024-03-03', '개강', 'blue') ;
update unischedule set stdate = '2024-03-03', sccontent = '개강', scolor = '#fdfdfd' where scno =8;
delete from unischedule where scno = 7;

# =========================== 청원 댓글 =========================== #
drop table if exists preply;
create table preply(
   mno int not null,               #사람 번호
    ppno int,               #청원게시물번호
    replyno int auto_increment,         #댓글 번호
    replycontent text not null,         #댓글 내용
    pposition bigint unsigned default 0 not null,
    primary key(replyno),
    constraint petition_ppno_fk foreign key(ppno) references petition(ppno) on update cascade on delete cascade
);
select * from preply;